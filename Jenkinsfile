pipeline {
    ...
    stages{
        ...
        stage('Build docker image'){
            steps{
                script{
                    sh 'docker build -t kubergyan1/php-project:v1 .'
                    sh 'docker images'
                }
            }
        }
          stage('Docker login') {
            steps {
                withCredentials([usernamePassword(credentialsId: 'dockerhub-cred', passwordVariable: 'PASS', usernameVariable: 'USER')]) {
                    ...
                    sh 'docker push kubergyan1/php-project:v1'
                }
            }
        }
            ...
            steps {
               script {
                   def dockerrm = 'sudo docker rm -f My-first-containe2211 || true'
                    def dockerCmd = 'sudo docker run -itd --name My-first-containe2211 -p 8083:80 kubergyan1/php-project:v1'
                    sshagent(['sshkeypair']) {
                        //chnage the private ip in below code
                        // sh "docker run -itd --name My-first-containe2111 -p 8083:80 kubergyan1/php-project:v1"
